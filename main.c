...
/* バージョン文字列 */
#define AADCUI_VERSION_STRING  "1.0.0"

/* ブロックサイズ 今の所1024で固定 */
#define AADCUI_BLOCK_SIZE      1024
...
static int do_residual_output(const char *wav_file, const char *residual_filename)
{
  ...
  for (ch = 0; ch < num_channels; ch++) {
    for (smpl = 0; smpl < num_samples; smpl++) {
      pcmdata[ch][smpl] = (int32_t)(WAVFile_PCM(wavfile, smpl, ch) >> 16);
    }
  }
  ...
  if ((api_result = AADDecoder_DecodeWhole(decoder, 
        buffer, output_size, pcmdata, num_channels, num_samples)) != AAD_APIRESULT_OK) {
    fprintf(stderr, "Failed to decode. API result: %d \n", api_result);
    return 1;
  }
...
{
  printf(
      "AAD(Ayashi Adaptive Differential pulse code modulation) encoder/decoder Version." AADCUI_VERSION_STRING "\n" \
      "Usage: %s -[edr] INPUT OUTPUT \n" \
      "-e: encode mode (wav file -> .aad file)\n" \
      "-d: decode mode (.aad file -> wav file)\n" \
      "-r: output residual (wav file -> Residual wav file)\n", 
      program_name);
}
